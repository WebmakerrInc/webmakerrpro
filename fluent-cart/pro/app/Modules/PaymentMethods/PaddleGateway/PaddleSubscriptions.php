...
class PaddleSubscriptions extends AbstractSubscriptionModule
{
    ...
    public function reSyncSubscriptionFromRemote(Subscription $subscriptionModel)
    {
        if ($subscriptionModel->current_payment_method !== 'paddle') {
            return new \WP_Error(
                ...
                __('This subscription is not using Paddle as payment method.', 'fluent-cart')
            );
        }
        ...
        if (!$vendorSubscriptionId) {
            return new \WP_Error(
                ...
                __('Invalid vendor subscription ID.', 'fluent-cart')
            );
        }
    ...
    public function pauseSubscription($data, $order, $subscription)
    {
        ...
        if (!$vendorSubscriptionId) {
            throw new \Exception(__('Invalid vendor subscription ID.', 'fluent-cart'), 404);
        }
        ...
        return [
            'status' => 'success',
            'message' => __('Subscription paused successfully', 'fluent-cart'),
    ...
    public function resumeSubscription($data, $order, $subscription)
    {
        ...
        return [
            'status' => 'success',
            'message' => __('Subscription resumed successfully', 'fluent-cart'),
            'paddle_response' => $response
        ];
    }
    ...
    public function switchPaymentMethod($data, $subscriptionId)
    {
       new \WP_Error(
            'not_supported',
            __('Payment method switching is not directly supported by Paddle. Please cancel and recreate the subscription.', 'fluent-cart')
        );
    }
    ...
    public function reactivateSubscription($data, $subscriptionId)
    {
       ...
       new \WP_Error(
            'not_supported',
            __('Subscription reactivation is not directly supported by Paddle. Please create a new subscription.', 'fluent-cart')
        );
    }
}
