    ...
    public function getLicenses(Request $request)
    {
        ...
        if (!$customer) {
            return $this->sendSuccess([
                'message'  => __('Unable to find licenses', 'fluent-cart'),
                'licenses' => [
                    'data'  => [],
                    'total' => 0,
                ]
    ...
    public function deactivateSite(Request $request, $licenseKey)
    {
        ...
        if ($site) {
            ...
            if ($activation) {
                ...
                return $this->sendSuccess([
                    'message' => __('Site deactivated successfully', 'fluent-cart')
                ]);
            }
        }

        return $this->sendError([
            'message' => __('Site not found or not activated for this license', 'fluent-cart')
        ], 422);
    }
