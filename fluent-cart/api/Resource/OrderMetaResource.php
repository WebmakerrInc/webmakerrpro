...
class OrderMetaResource extends BaseResourceApi
{
    ...
    public static function delete($id, $params = [])
    {
        ...
        if ($orderMeta) {
            if ($orderMeta->delete()) {
                static::makeSuccessResponse('', sprintf(__("%s successfully deleted.", 'fluent-cart'), $metaKey));
            }

            return static::makeErrorResponse([
                ['code' => 400, 'message' => sprintf(__('%s deletion failed!', 'fluent-cart'), $metaKey)]
            ]);
        }

        return static::makeErrorResponse([
            ['code' => 404, 'message' => sprintf(__('%s not found in database, failed to remove.', 'fluent-cart'), $metaKey)]
        ]);
    }
