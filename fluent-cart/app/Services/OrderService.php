...
class OrderService
{
    ...
    public static function validateProducts($products)
    {
        ...
        foreach ($products as $product) {
            ...
            static::validateStockStatus($product, $currentVariations);
            static::validateStockQuantity($product, $currentVariations);
        }
    }
    ...
    private static function validateStockStatus($product, $currentVariations)
    {
        $currentVariation = $currentVariations[$product['object_id']] ?? null;
    ...
    private static function validateStockQuantity($product, $currentVariations)
    {
        $currentVariation = $currentVariations[$product['object_id']] ?? null;
        ...
        if (!static::allowItemToOrder($currentVariation, $productQuantity)) {
            $note = sprintf(
                /* translators: %s: product title */
                esc_html__('[%s] is out of stock. Only %s left.', 'fluent-cart'),
                esc_html(Arr::get($product, 'title')),
