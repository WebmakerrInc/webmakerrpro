...
?>
<div class="fca_admin_menu_wrap fca_global_menu_wrap fluent-cart-admin-pages">
    <div class="fca_admin_menu_row">
        <div class="fca_admin_logo_wrap">
            <a aria-label="<?php echo __('WebmakerrPro Logo', 'fluent-cart'); ?>"
               title="<?php echo __('WebmakerrPro Logo', 'fluent-cart'); ?>"
               href="<?php echo esc_url(admin_url('admin.php?page=fluent-cart#/')); ?>">
                <img class="block dark:hidden" src="<?php echo esc_url($lightLogo); ?>"
                     alt="<?php echo __('WebmakerrPro Logo', 'fluent-cart'); ?>"/>
                <img class="hidden dark:block" src="<?php echo esc_url($darkLogo); ?>"
                     alt="<?php echo __('WebmakerrPro Logo', 'fluent-cart'); ?>"/>
            </a>
        </div><!-- .fca_admin_logo_wrap -->

        <div class="fca_admin_menu">
            <ul class="fc_menu">
                <?php foreach ($menu_items as $itemSlug => $menu_item):
                    if (empty($menu_item['permission']) || PermissionManager::hasPermission($menu_item['permission'])):
                        ?>
                        <li class="fc_menu_item fc_menu_item_<?php echo esc_attr($itemSlug);
                        echo !empty($menu_item['children']) ? ' has-child' : ''; ?>">
                            ...
                            <?php if (!empty($menu_item['children'])): ?>
                                <button aria-label="<?php echo esc_attr($menu_item['label']) ?>">
                                    ...
                                    <span class="fc_menu_down_arrow">
                                        <svg class="w-64 h-64" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                                             fill="currentColor">
                                ...
                                <ul class="fc_menu_child">
                                    <?php foreach ($menu_item['children'] as $childSlug => $childItem):
                                        if (empty($childItem['permission']) || PermissionManager::hasPermission($childItem['permission'])):
                                            ?>
                                            <li class="fc_menu_child_item fc_menu_child_item_<?php echo esc_attr($childSlug); ?>">
                                                <a type="button"
        ...
        <div class="fca_admin_menu_actions">
            <div id="fc_admin_menu_search"></div>
            <div class="fc_admin_menu_button_wrap">
                <div id="theme-button-container"></div>
                <?php if (PermissionManager::hasPermission(["store/settings", 'store/sensitive'])): ?>
                    <div class="fc_admin_settings_btn_wrap">
                        <a class="fc_admin_settings_btn"
                           href="<?php echo admin_url('admin.php?page=fluent-cart#/settings/store-settings/') ?>">
                ...
                <?php endif; ?>
                <div class="fc-mobile-menu-container" id="mobile-menu-container">
                    <div class="icon w-5 h-5" data-fct-menu-toggle>
                    ...
                    <div class="fc-offcanvas-menu" data-fc-offcanvas-menu>
                        <button class="fc-offcanvas-menu-close" data-fc-offcanvas-menu-close>
                        ...
                        <div class="fc-offcanvas-menu-list">
                            <?php foreach ($menu_items as $itemSlug => $menu_item): ?>
                                <?php if (!empty($menu_item['children'])): ?>
                                    <?php foreach ($menu_item['children'] as $childSlug => $childItem): ?>
                                        <div class="fc-offcanvas-menu-item">
                                            <div class="fc-offcanvas-menu-label">
        ...
        <!--        <div class="fca_admin_menu_help">-->
        <!--            <a href="#" target="_blank" class="fca_admin_menu_help_link">-->
        <!--                --><?php //echo esc_html__('Need Help?', 'fluent-cart'); ?>
        <!--            </a>-->
        <!--        </div>-->
    </div>
</div>
