...
use FluentCart\App\Services\ConditionAssesor;
...
trait MailChimpSubscriber
{
     ...
     * @var string
     */
    protected $metaKey = 'fluent_cart_mailchimp_feed';
     ...
     * @throws \Exception
     */
    public function subscribe($feed, $formData, $entry)
    {
        ...
        $status = Helper::isTrue($feedData, 'doubleOptIn') ? 'pending' : 'subscribed';
        ...
        $arguments = [
            ...
            'double_optin' => Helper::isTrue($feedData, 'doubleOptIn'),
            'vip' => Helper::isTrue($feedData, 'markAsVIP'),
        ];
        ...
        if (Arr::get($entry, 'ip', false)) {
            $arguments['ip_signup'] = Arr::get($entry, 'ip');
        }
