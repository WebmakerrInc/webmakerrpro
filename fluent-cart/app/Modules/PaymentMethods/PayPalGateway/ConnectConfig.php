...
class ConnectConfig
{
    ...
    public static function parseConnectInfos($vendorData)
    {
        if (!$vendorData || !Arr::get($vendorData, 'permissionsGranted')) {
            echo '<div class="fct_message fct_message_error">' . esc_html(__('Invalid PayPal Request. Please try configuring paypal payment gateway again!')) . '</div>';
            die();
        }
        ...
        */
        ...
        wp_redirect(admin_url('admin.php?page=webmakerrpro#/settings/payments/paypal'));
    }
    ...
    public static function verifyAuthorizeSuccess($data)
    {
        ...
        if (empty($response['paypal_user_id'])) {
            ...
            if (!$message) {
                $message = __('Invalid PayPal Request. Please configure paypal payment gateway again');
            }
            echo '<div class="fct_message fct_message_error">' . esc_html($message) . '</div>';
            return;
        }
    ...
    public static function disconnect($mode, $sendResponse = true)
    {
        ...
        if (empty($paypalSettings[$mode . '_account_id'])) {
            if ($sendResponse) {
                wp_send_json([
                    'message' => __('Selected Account does not exist')
                ], 422);
            }
            return false;
        }
        ...
        if ($sendResponse) {
            wp_send_json([
                'message'  => __('PayPal settings has been disconnected'),
