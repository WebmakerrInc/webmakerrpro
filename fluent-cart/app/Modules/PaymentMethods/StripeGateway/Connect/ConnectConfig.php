...
class ConnectConfig
{
    ...
    public static function getConnectConfig(): array
    {
        ...
        return [
            'connect_config' => [
                'live_redirect' => home_url('?fluent-cart=stripe_connect&intent=connect&mode=live'),
                'test_redirect' => home_url('?fluent-cart=stripe_connect&intent=connect&mode=test'),
    ...
    public static function verifyAuthorizeSuccess($data)
    {
        ...
        if (empty($response['stripe_user_id'])) {
            ...
            if (!$message) {
                $message = __('Invalid Stripe Request. Please configure stripe payment gateway again');
            }
            echo '<div class="fct_message fct_message_error">' . esc_html($message) . '</div>';
            return;
        }
    ...
    public static function disconnect($mode, $sendResponse = true)
    {
        ...
        if ($sendResponse) {
            wp_send_json([
                'message'  => __('Stripe settings has been disconnected'),
