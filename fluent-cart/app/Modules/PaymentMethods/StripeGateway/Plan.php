...
class Plan
{
    ...
    public static function getStripePricing($data = [])
    {
        ...
        $data['billing_interval'] = self::translateToStripeInterval(Arr::get($data, 'billing_interval'));

        $sitePrefix = Helper::getSitePrefix();
        $pricingId = 'fct_' . $sitePrefix . '_price_' . $data['variation_id'] . '_' . $data['recurring_total'] . '_' . $data['billing_interval'] . '_' . $data['interval_count'] . '_' . $data['trial_days'] . '_' . $data['currency'];
    ...
    public static function translateToStripeInterval($billingInterval)
    {
