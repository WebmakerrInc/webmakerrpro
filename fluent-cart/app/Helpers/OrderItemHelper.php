...
class OrderItemHelper
{
    ...
    {
        if (empty($items)) {
            throw new \Exception(__('Please add some products!', 'fluent-cart'));
        }

        if (!$orderId) {
            throw new \Exception(__('Order Not valid!', 'fluent-cart'));
        }
    ...
    public function processCustom($product, $orderId)
    {
        ...
        if (!Arr::get($product, 'item_name', false)) {
            throw new \Exception(__('Item must have a name!', 'fluent-cart'));
        }

        if (!$price || !$quantity) {
            throw new \Exception(__('Price, Quantity field should not be empty or zero!', 'fluent-cart'));
        }
