...
class FluentCartGeneralApi
{
    ...
    {
        if (!$slug) {
            throw new \Exception(__('The endpoint slug cannot be empty.', 'fluent-cart'));
        }

        $reserved = ['dashboard', 'purchase-history', 'subscriptions', 'licenses', 'downloads', 'profile'];

        if (in_array($slug, $reserved)) {
            throw new \Exception(sprintf(__('The endpoint slug "%s" is reserved and cannot be used.', 'fluent-cart'), $slug));
        }

        if (!isset($args['render_callback']) && !isset($args['page_id'])) {
            throw new \Exception(__('You must provide either a render callback or a page ID for the endpoint.', 'fluent-cart'));
        }
    ...
    public function registerCustomPaymentMethod($name, $paymentGatewayInstance)
    {
        if(! $paymentGatewayInstance instanceof AbstractPaymentGateway) {
            throw new \Exception(sprintf(__('The payment gateway class "%s" is not valid. It must extend AbstractPaymentGateway.', 'fluent-cart'), $paymentGatewayClass));
        }
