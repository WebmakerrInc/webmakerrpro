...
class OrderResource extends BaseResourceApi
{
    ...
    private static function commitEvents($order)
    {
        ...
        if (!$order) {
            throw new \Exception(__('Please process order first', 'fluent-cart'));
        }

        if (!$order->customer) {
            throw new \Exception(__('Please set customer first', 'fluent-cart'));
        }

        if (!$order->latest_transaction) {
            throw new \Exception(__('Please set Transaction First', 'fluent-cart'));
        }
...
//            (new StockChanged($productIds))->dispatch();
     ...
     *                    'object_id'      => ( int ) The product ID associated with the order item.
     *                    'variation_id'   => ( int ) The variation ID of the order item.
    ...
    public static function update($data, $id, $params = [])
    {
        ...
        if (empty($order) || $order->status === Status::ORDER_COMPLETED || $order->status === Status::ORDER_CANCELED) {
            ...
            return static::makeErrorResponse([
                ['code' => 404, 'message' => sprintf(__('Your order status is marked as %s and not eligible for any further modifications at this time.', 'fluent-cart'), $order->status)]
            ]);
        }
    ...
    public static function updateStatuses(array $params = [])
    {
        ...
        if ($action === 'change_shipping_status') {
            ...
            $actionActivity = [
                ...
                'content' => sprintf(__('Shipping status has been updated from %1$s to %2$s', 'fluent-cart'), $oldStatus, $newStatus)
            ];
        ...
        if ($action === 'change_order_status') {
            ...
            $actionActivity = [
                ...
                'content' => sprintf(__('Order status has been updated from %1$s to %2$s', 'fluent-cart'), $oldStatus, $newStatus)
            ];
        }
                    ...
                    if ($action === 'change_shipping_status') {
                        ...
                        if ($action !== 'change_shipping_status' && Arr::get($params, 'manage_stock') == 'true') {
                            ...
                            if (Arr::get($validationSucceeded, 'status') === true) {
                                return static::makeErrorResponse([
                                    ['code' => 400, 'message' => sprintf(__('%s', 'fluent-cart'), Arr::get($validationSucceeded, 'message'))]
                                ]);
                            }
                        }
    ...
    private static function validateStock($orderItems)
    {
        ...
        if (!empty($outOfStockVariants)) {
            $message = (count($outOfStockVariants) > 1)
                ? sprintf(__('%s are out of stock', 'fluent-cart'), implode(', ', $outOfStockVariants))
                : sprintf(__('%s is out of stock', 'fluent-cart'), reset($outOfStockVariants));
    ...
    public static function bulkDeleteByOrderIds($orderIds, $params = [])
    {
        ...
        if (count($failedOrderIds) > 0) {
            ...
            return count($deletedOrderIds) > 0
                ? static::makeSuccessResponse('', sprintf(__("The order ID - %s cannot be deleted at the moment as these orders status is not canceled. And remaining order and its associated data have been deleted", 'fluent-cart'), $failedOrderIds))
                : static::makeErrorResponse([['code' => 400, 'message' => sprintf(__("The order ID - %s cannot be deleted at the moment as these orders status is not canceled.", 'fluent-cart'), $failedOrderIds)]]);
        }
    ...
    public static function updatePaymentStatus(array $params = [])
    {
        ...
        if (!empty($data) && $updatedStatus) {
            ...
            $actionActivity = [
                ...
                'content' => sprintf(__('Payment status has been updated from %1$s to %2$s', 'fluent-cart'), $oldStatus, $newStatus)
            ];
