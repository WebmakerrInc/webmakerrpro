...
//            'total_amount'          => $this->orderTotals['total_amount'],
    ...
    private function calculateTrialDaysForInterval($trialDays, $repeatInterval)
    {
        ...
        switch ($repeatInterval) {
            case 'daily':
                // For daily billing, trial should be at least 1 day, max reasonable is 7-14 days
                return min(max($trialDays, 1), 30); // Cap at 30 days for daily billing

            case 'weekly':
                // For weekly billing, trial should be in multiples of 7 days ideally
                // But we'll accept any reasonable number
                return min($trialDays, 90); // Cap at ~13 weeks

            case 'monthly':
                // For monthly billing, common trials are 7, 14, 30 days
                return min($trialDays, 90); // Cap at 3 months worth of days

            case 'yearly':
                // For yearly billing, trial can be much longer
                return min($trialDays, 365); // Cap at 1 year
