...
class CouponHelper
{
    ...
    public static function checkStackability($appliedCoupon, $appliedCouponList, $coupon)
    {
        ...
        if (in_array($appliedCoupon, $appliedCouponList)) {
            return
                ['code' => 400, 'message' => sprintf(__('%s coupon can only be applied once per order', 'fluent-cart'), $appliedCoupon)];
        }
        ...
        if (count($appliedCouponList) > 0) {
            ...
            if ($lastCouponStackability == 'no') {
                return
                    ['code' => 400, 'message' => sprintf(__('%s coupon cannot be used with other coupon', 'fluent-cart'), end($appliedCouponList))];
            }
        }
        ...
        if (count($appliedCouponList) > 0 && $coupon->stackable == 'no') {
            ...
            return [
                'code' => 400,
                'message' => sprintf(
                    __('This %1$s coupon cannot be used with other coupon', 'fluent-cart'),
                    $appliedCoupon // %1$s: Applied Coupon (e.g., "DISCOUNT10")
                )
            ];
    ...
    public static function checkProductEligibility($productId, $couponCode, $origin)
    {
         ...
         */
        $message = sprintf(
            __('%1$s conflicts with %2$s coupon. Remove the coupon first.', 'fluent_cart'),
        ...
        if ($includedCategories !== null && empty(array_intersect($includedCategories, $productCategories))) {
            return [
                'isApplicable' => false,
                'message' => __('This coupon can not be applied to some items in your cart', 'fluent_cart'),
            ];
        }
        return [
            'isApplicable' => true,
