    ...
    public function processPaypalWebhookEvents($event): void
    {
        ...
        if (empty($resource)) {
            if (defined('FLUENT_CART_DEV_MODE')) {
                error_log('PayPal webhook resources is empty!');
            }
            return;
        }
...
//            'payment_method_type' => Arr::get($charge, 'payee.email_address', 'PayPal'),
    ...
    public function processWebhook()
    {
        ...
        if (!$data) {
            if (defined('FLUENT_CART_DEV_MODE')) {
                error_log('PayPal Webhook Received with empty data: ' . $post_data);
            }
    ...
    public function handleWebhookDisputeCreated($data)
    {
        ...
        if (count($disputedTransactions) > 1) {
            if (defined('FLUENT_CART_DEV_MODE')) {
                error_log('PayPal dispute has multiple transactions: ' . json_encode($disputedTransactions));
            }
            return false;
        }
