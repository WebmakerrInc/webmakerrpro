...
class PayPal extends AbstractPaymentGateway
{
    ...
    {
        if (empty($_REQUEST['payId']) || empty($_REQUEST['ref_id'])) {
            wp_send_json([
                'status'  => 'failed',
        ...
        $payPalReferenceId = sanitize_text_field($_REQUEST['payId']);
        $transactionHash = sanitize_text_field($_REQUEST['ref_id']);
    ...
    public function confirmPayPalSubscription()
    {
        ...
        $transaction = OrderTransaction::query()->where('uuid', sanitize_text_field($_REQUEST['ref_id']))->first();
    ...
    public static function validateSettings($data): array
    {
        ...
        if (!$clientId || !$clientSecret) {
            return [
                'status'  => 'failed',
                'message' => __($mode === 'live' ? __('PayPal live credentials is required!', 'fluent-cart') : __('PayPal test credentials is required!!', 'fluent-cart'), 'fluent-cart')
            ];
        }
