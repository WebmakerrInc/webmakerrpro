...
trait CanCalculateSubscriptionCountTrend
{
    ...
    protected function get_total_subscription_counts_for_multiple_dates(array $snapshot_dates, $offsetMinutes = 0, $currency = null): array
    {
        ...
        $query = $wpdb->prepare(
            "SELECT
                s.id,
                s.created_at + INTERVAL %d MINUTE as created_at,   -- Subscription start date (timezone adjusted)
                s.expire_at + INTERVAL %d MINUTE as expire_at,     -- End of fixed term/trial (timezone adjusted)  
                s.canceled_at + INTERVAL %d MINUTE as canceled_at, -- Explicit cancellation date (timezone adjusted)
                s.status
            FROM {$table_subscriptions} s
            INNER JOIN {$table_orders} o ON s.parent_order_id = o.id
            WHERE s.status IN (%s, %s, %s) -- Consider 'active', 'trialling', 'pending' for active counts
            AND (
                s.created_at <= %s
                AND (
                    s.expire_at IS NULL OR s.expire_at >= %s
                )
                AND (
                    s.canceled_at IS NULL OR s.canceled_at > %s
                )
            )
            {$currency_filter}",
            $offsetMinutes, $offsetMinutes, $offsetMinutes,
            'active', 'trialling', 'pending',
