...
trait CanCalculateChurnRevenue
{
    ...
    public function get_churn_revenue_trend($period_start_date_str = null, $period_end_date_str = null, $interval_type = 'monthly', $currency = null)
    {
        ...
        $end_date_obj = DateTime::anyTimeToGmt($period_end_date_str ?: (defined('static::db_max_date') ? static::$db_max_date : date('Y-m-d H:i:s')));
    ...
    protected function get_churn_revenue_events_in_range(string $range_start_gmt, string $range_end_gmt, $currency = null): array
    {
        ...
        // Fetch subscriptions that have 'churned' within the specified date range.
        // Churn date is determined by COALESCE(canceled_at, expire_at) for statuses like 'cancelled', 'expired', 'failed'.
        // Columns: recurring_amount, billing_interval, status, canceled_at, expire_at
        $churned_subscriptions = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT
                    id,
                    recurring_amount,
                    billing_interval,
                    status,
                    COALESCE(canceled_at, expire_at) as churn_date -- Use canceled_at if available, else expire_at
                FROM {$table_subscriptions}
                WHERE status IN (%s, %s, %s)
                AND (COALESCE(canceled_at, expire_at) >= %s AND COALESCE(canceled_at, expire_at) <= %s)
                {$currency_filter}",
                'cancelled', 'expired', 'failed', // Common churn statuses
                $range_start_gmt,
                $range_end_gmt
            ),
            ARRAY_A
