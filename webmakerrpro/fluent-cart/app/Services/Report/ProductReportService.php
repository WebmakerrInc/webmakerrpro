...
{    
    public function getProductTopChart(array $params)
    {
        $orderItemsSubQuery = App::db()->table('fct_order_items')
            ->selectRaw('order_id, object_id, title, post_title, SUM(quantity) AS sum_quantity')
        ...
        $query = App::db()->table('fct_orders as o')
            ...
            ->selectRaw("
                ...
                SUBSTRING_INDEX(GROUP_CONCAT(oi.title ORDER BY o.created_at DESC SEPARATOR '||'), '||', 1) AS latest_title,

                SUBSTRING_INDEX(GROUP_CONCAT(oi.post_title ORDER BY o.created_at DESC SEPARATOR '||'), '||', 1) AS latest_post_title
            ")
