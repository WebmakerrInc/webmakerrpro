...
class OrderCloneCommand
{
    ...
    public function clone_orders($args, $assoc_args)
    {
        ...
        $startDate = isset($assoc_args['start-date']) ? $assoc_args['start-date'] : date('Y-m-d', strtotime('-30 days'));
        $endDate = isset($assoc_args['end-date']) ? $assoc_args['end-date'] : date('Y-m-d');
    ...
    private function cloneSubscriptions($sourceOrder, $newOrder)
    {
        ...
        foreach ($subscriptions as $subscription) {
            ...
            if ($subData['next_billing_date']) {
                ...
                $subData['next_billing_date'] = date('Y-m-d H:i:s',
    ...
    private function getRandomDateInRange($startDate, $endDate)
    {
        ...
        $randomTime = mt_rand(0, 86399); // 0 to 23:59:59

        return date('Y-m-d H:i:s', $randomTimestamp + $randomTime);
    }
