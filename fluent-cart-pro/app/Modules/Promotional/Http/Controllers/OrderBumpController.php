...
class OrderBumpController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (empty($title) || empty($srcObjectId)) {
            return $this->sendError([
                'message' => __('Title and source object id are required', 'fluent-cart')
            ], 400);
        }
        ...
        if (!$isCreated) {
            return $this->sendError([
                'message' => __('Failed to create order bump', 'fluent-cart')
            ], 400);
        }

        return [
            'message' => __('Order bump created successfully', 'fluent-cart'),
            'id'      => $isCreated->id
        ];
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        if (!$orderBump) {
            return $this->sendError([
                'message' => __('Order bump not found', 'fluent-cart')
            ], 404);
        }
    ...
    public function update(OrderBumpRequest $request, $id)
    {
        ...
        if (!$isUpdated) {
            return $this->sendError([
                'message' => __('Failed to update order bump', 'fluent-cart')
            ], 400);
        }

        return [
            'message' => __('Order bump updated successfully', 'fluent-cart')
        ];
    }
    ...
    public function delete(Request $request, $id)
    {
        ...
        if (!$isDeleted) {
            return $this->sendError([
                'message' => __('Failed to delete order bump', 'fluent-cart')
            ], 400);
        }

        return [
            'message' => __('Order bump deleted successfully', 'fluent-cart')
        ];
    }
