    ...
    public function confirmPaddlePayment()
    {
        ...
        if ($transactionModel->status === Status::TRANSACTION_SUCCEEDED) {
            wp_send_json([
                ...
                'message' => __('Payment already confirmed. Redirecting...!', 'fluent-cart'),
                'status' => 'success'
            ], 200);
        }
        ...
        if ($transactionStatus !== 'paid' && $transactionStatus !== 'completed') {
            wp_send_json([
                'message' => sprintf(__('Payment is not completed. Current status: %s', 'fluent-cart'), $transactionStatus),
                'status' => 'failed'
            ], 400);
        }
