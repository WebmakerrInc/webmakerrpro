...
class PaddleHelper
{
    ...
    public static function processRemoteRefund($transaction, $amount, $args)
    {
        ...
        if ($totalRefundableAmountFromItems < $amount && !empty($itemIds)) {
            return new \WP_Error('invalid_refund', __('Invalid refund amount. Please check the items available refund amounts.', 'fluent-cart'));
        ...
        if (!in_array($status, $acceptedStatus)) {
            return new \WP_Error('refund_failed', __('Refund could not be processed in paddle. Please check on your paddle account', 'fluent-cart'));
        }
