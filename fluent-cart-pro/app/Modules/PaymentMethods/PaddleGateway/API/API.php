...
class API
{
    ...
    public static function makeRequest($endpoint, $data = [], $method = 'POST', $mode = '')
    {
        ...
        if (empty($apiKey)) {
            return new \WP_Error(
                ...
                __('Paddle API key is missing for mode: ' . $mode, 'fluent-cart')
            );
        }
    ...
    public static function verifyWebhookSignature($payload, $signature, $mode = '')
    {
        ...
        if (empty($webhookSecret)) {
            return new \WP_Error(
                ...
                __('Paddle webhook secret is missing', 'fluent-cart')
            );
        }
        ...
        if (!hash_equals($expectedSignature, $signature)) {
            return new \WP_Error(
                ...
                __('Invalid webhook signature', 'fluent-cart')
            );
        }
    ...
    private static function parseErrorMessage($responseBody)
    {
        ...
        return __('Unknown Paddle API error', 'fluent-cart');
    }
